...
import { GET_POKEMON_IMG } from '@/queries/pokemonQueries';
...
export default function DetailsLeft({
	...
	const { loading, error, data } = useQuery(GET_POKEMON_IMG, {
		variables: {
			limit: 1,
			offset: id - 1,
		},
	...
	const img_src =
		data?.pokemons?.results[0]?.artwork ||
		data?.pokemons?.results[0]?.dreamworld;
	...
	return (
		<div className="flex flex-col-reverse lg:flex-row w-full md:w-2/3 items-center justify-between">
			<div className="w-full lg:w-1/2 pr-6">
				<h1 className="text-[#257BC4] font-medium text-5xl text-center lg:text-left">
					{name} #{id}
				</h1>
